...
public class SQDataSource {
	...
	public long insert(DietChart eChart) {
		...
		values.put(dietDBhelper.COLUMNL_NAME_FIELD, eChart.getEventName());
		values.put(dietDBhelper.COLUMNL_DATE_FIELD, eChart.getDate());
		values.put(dietDBhelper.COLUMNL_TIME_FIELD, eChart.getTime());
		values.put(dietDBhelper.COLUMNL_MENU_FIELD, eChart.getMenu());

		long inserted = dietDataBase.insert(dietDBhelper.TABLE_NAME, null,
				values);
		dietDataBase.close();
		return inserted;
	}
	...
	public long updateData(Integer id, DietChart eChart) {
		...
		try {
			updated = dietDataBase.update(dietDBhelper.TABLE_NAME, values,
					dietDBhelper.COLUMNL_ID_FIELD + "=" + id, null);
	...
	public ArrayList<DietChart> getDietProfiles() {
		...
		Cursor cursor = dietDataBase.query(dietDBhelper.TABLE_NAME, null,
				dietDBhelper.COLUMNL_DATE_FIELD + "= '" + mCurrentDate + "' ",
				null, null, null, null);
		...
		if (cursor != null && cursor.getCount() > 0) {
			...
			for (int i = 0; i < cursor.getCount(); i++) {
				...
				int id = cursor.getInt(cursor
						.getColumnIndex(dietDBhelper.COLUMNL_ID_FIELD));
				String name = cursor.getString(cursor
						.getColumnIndex(dietDBhelper.COLUMNL_NAME_FIELD));
				String date = cursor.getString(cursor
						.getColumnIndex(dietDBhelper.COLUMNL_DATE_FIELD));
				String time = cursor.getString(cursor
						.getColumnIndex(dietDBhelper.COLUMNL_TIME_FIELD));
				String menu = cursor.getString(cursor
						.getColumnIndex(dietDBhelper.COLUMNL_MENU_FIELD));
	...
	public ArrayList<DietChart> getAllDates() {
		...
		Cursor cursor = dietDataBase.query(dietDBhelper.TABLE_NAME, null, null,
				null, null, null, null);
